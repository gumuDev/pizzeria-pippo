-- ============================================================
-- 033_create_order_atomic.sql
-- Función transaccional para confirmar ventas.
--
-- Problema que resuelve: /api/orders hacía 4 pasos secuenciales
-- (orden → items → sabores → stock) sin transacción; un fallo
-- intermedio dejaba órdenes huérfanas o stock sin descontar.
--
-- Ahora el backend calcula (precios, promos, deducciones) y esta
-- función aplica TODO en una sola transacción: o entra completo
-- o no entra nada.
--
-- Bonus: el daily_number se genera con un advisory lock por
-- sucursal+día, eliminando la race condition de ventas simultáneas.
-- ============================================================
...
BEGIN
  ...
  INSERT INTO orders (branch_id, cashier_id, total, daily_number, payment_method, order_type, idempotency_key)
  VALUES (v_branch_id, v_cashier_id, v_total, v_daily_number, v_payment_method, v_order_type, v_idempotency_key)
